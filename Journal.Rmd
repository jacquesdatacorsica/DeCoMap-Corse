...
```{r, echo=TRUE,  cache=TRUE}
enquête =BOAMP[which(grepl("sologne", BOAMP$denomination_ach, ignore.case = TRUE)==TRUE |grepl("beuvron", BOAMP$denomination_ach, ignore.case = TRUE)==TRUE |grepl("neung", BOAMP$ville_ach, ignore.case = TRUE)==TRUE),]
table=enquête[,c(1,3,10,23)]
table%>%datatable(extensions = 'Buttons',
          options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                          lengthMenu = list(c(10,25,50,-1),
...
enquête =df[which(grepl("sologne", df$acheteur.nom, ignore.case = TRUE)==TRUE |grepl("beuvron", df$acheteur.nom, ignore.case = TRUE)==TRUE |grepl("neung", df$acheteur.nom, ignore.case = TRUE)==TRUE),]
table=enquête[,c(3,6,13,36)]
table%>%datatable(extensions = 'Buttons',
          options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                          lengthMenu = list(c(10,25,50,-1),
